    ...
    public static class WmlToHtmlConverter
    {
        ...
        private static bool HasAPageBreak(XElement element)
        {
            if (element != null && element.Descendants() != null)
            {
                ...
                if (validElements.Count == 0) return false;
                var found = validElements.Any(dl => dl != null && dl.HasAttributes && dl.Attribute(W.type).Value != null && dl.Attribute(W.type).Value == "page");
